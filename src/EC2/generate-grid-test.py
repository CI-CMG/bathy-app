...
def main():
    ...
    while True:
        ...
        for message in messages:
            ...
            grid_params = body['grid']
            datasets = body['datasets']
            # location:filename KVPs. temporarily ignore multibeam datasets
            output_locations = {dataset['label']: dataset['output_location'] for dataset in datasets if
                                dataset['label'] != 'multibeam'}
            bbox = body['bbox']
            data_files = []

            try:
                logger.info("preparing mbgrid execution...")
                ...
                logger.info("packaging results into zip file...")
                zip_filename = zip_files(order_id, data_files)

                # upload to S3 delivery bucket
                logger.info("uploading zip file to delivery S3 bucket...")
                s3_key = upload_file_to_s3(zip_filename)

                payload = {
                    'output_location': f's3://{OUTPUT_BUCKET}/{order_id}.zip',
                    'order_id': order_id,
                    'datasets': datasets
                }
                logger.info('returning payload:\n')
                logger.info(json.dumps(payload))
                logger.info("notifying step function of success...")
                send_success(task_token, payload)

            except Exception as e:
                logger.error(e)
                logger.error('Failed to process message and will not retry')
                send_failure(task_token, error_cause='failed to generate grid')
                ...
                logger.info(f'deleting message for order_id {order_id}')
                message.delete()
                logger.info("cleaning up data_files...")
...
def convert_csv_to_xyz(filename):
    ...
    with open(filename) as file_handler:
        ...
        for line in read_large_file(file_handler):
            ...
            x = str(round(values[0], 5))
            y = str(round(values[1], 5))
            z = str(round(values[2], 1))
            output_file.write(','.join([x, y, z]) + "\n")
    output_file.close()
    return output_filename
...
if __name__ == '__main__':
    ...
    QUEUE_NAME = 'GridDataQueue-test'
    ORDERS_TABLE = 'bathy-orders-test'
